...
@implementation UIImage (StyleTool)
//: + (UIImage *)fetchImage:(NSString *)imageNameOrPath{
+ (UIImage *)place:(NSString *)imageNameOrPath{
...
- (UIImage *)incentiveOption
{
    //: CGFloat pixels = [[NTESDevice currentDevice] suggestImagePixels];
    CGFloat pixels = [[OnTingHandle modernIndicator] completePixels];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self forBe:pixels];
...
- (UIImage *)forBe: (CGFloat)suggestPixels
{
...
- (UIImage *)cell: (CGFloat)maxPixels
{
    ...
    return [self doComplete:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)doComplete:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self shootLine:size];
}

//: - (UIImage *)drawImageWithSize: (CGSize)size
- (UIImage *)shootLine: (CGSize)size
{
