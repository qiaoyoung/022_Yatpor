...
- (void)visible: (UIView *)view containerHandler: (ActionSheetBlock)block
{
    ...
    else
    {
        ...
        else
        {
            ...
            [self performSelector:@selector(hid:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &k_imageId) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&k_imageId,nil,OBJC_ASSOCIATION_COPY);
