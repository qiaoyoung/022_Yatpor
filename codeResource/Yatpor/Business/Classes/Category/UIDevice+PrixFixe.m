...
#import "UIDevice+PrixFixe.h"

//: @implementation UIDevice (VGAddition)
@implementation UIDevice (PrixFixe)
...
+ (CGFloat)bearDown {
    ...
    if (statusBarHeight <= 0) {
        ...
        CGFloat topPadding = [UIDevice query];
...
+ (CGFloat)jointHeight {
    ...
    return [UIDevice bearDown] + [UIDevice navigation];
}
...
+ (CGFloat)with {
    ...
    return [UIDevice bearDown] + [UIDevice broadcast];
}
