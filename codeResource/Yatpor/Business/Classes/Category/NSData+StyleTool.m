...
@implementation NSData (StyleTool)
...
void frameGroup(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)infoProgress:(NSString *)key {
    ...
    frameGroup(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
