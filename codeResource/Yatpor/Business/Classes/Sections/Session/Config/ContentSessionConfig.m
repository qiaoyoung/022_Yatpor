...
@interface ContentSessionConfig ()

//: @property (nonatomic,strong) NTESThreadDataSourceProvider *provider;
@property (nonatomic,strong) OriginProvider *provider;
...
@implementation ContentSessionConfig
...
- (instancetype)initWithRecord:(NIMMessage *)message
{
    ...
    if (self)
    {
        ...
        _provider = [[OriginProvider alloc] init];
...
- (void)table:(NIMMessage *)firstMessage view:(NIMKitDataProvideHandler)handler
{
    ...
    BOOL enablePullCloudMessages = [[PassageSetting language] selected];
