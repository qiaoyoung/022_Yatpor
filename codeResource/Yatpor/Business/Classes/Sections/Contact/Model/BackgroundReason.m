...
- (instancetype)initWithStreetSmart:(id)first alongInitial:(id)second {
    //: self = [super init];
    self = [super init];
    //: if(self) {
    if(self) {
...
@implementation BackgroundReason
...
- (void)setMembers:(NSArray *)members
{
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        ...
        if (groupTitle.length) {
            ...
            [_groups addObject:[[Pair alloc] initWithStreetSmart:groupTitle alongInitial:groupedMembers]];
        }
    //: }];
    }];
    //: [self sort];
...
- (void)close:(id<TextTeam>)member
{
    ...
    if(!pair) {
        ...
        pair = [[Pair alloc] initWithStreetSmart:groupTitle alongInitial:members];
    }
...
- (void)item:(NSString *)title title:(NSArray *)members {
    ...
    Pair *pair = [[Pair alloc] initWithStreetSmart:title alongInitial:members];
...
- (void)length
{
    ...
    [_groups enumerateObjectsUsingBlock:^(Pair *obj, NSUInteger idx, BOOL *stop) {
        ...
        [groupedMembers sortUsingComparator:^NSComparisonResult(id<TextTeam> member1, id<TextTeam> member2) {
