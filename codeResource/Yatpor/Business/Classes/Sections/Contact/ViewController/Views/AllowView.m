...
NSString *StringFromProcessorData(Byte *data);        


//: #875FFA
Byte showValueIdent[] = {46, 7, 99, 6, 196, 156, 192, 213, 212, 210, 227, 227, 222, 103};

//: contact_my_tag_activity_my_friend
Byte user_backBubbleStr[] = {40, 33, 83, 10, 130, 10, 188, 2, 214, 254, 16, 28, 27, 33, 14, 16, 33, 12, 26, 38, 12, 33, 14, 20, 12, 14, 16, 33, 22, 35, 22, 33, 38, 12, 26, 38, 12, 19, 31, 22, 18, 27, 17, 127};

//: #5D5F66
Byte notiShouldId[] = {88, 7, 37, 6, 186, 189, 254, 16, 31, 16, 33, 17, 17, 170};

//: contact_fragment_group
Byte k_resourceData[] = {16, 22, 90, 13, 215, 120, 113, 230, 244, 134, 225, 50, 69, 9, 21, 20, 26, 7, 9, 26, 5, 12, 24, 7, 13, 19, 11, 20, 26, 5, 13, 24, 21, 27, 22, 109};
...
#import "UIButton+ObjectTitle.h"

//: @interface NTESContactHeaderView ()<UITextFieldDelegate>
@interface AllowView ()<UITextFieldDelegate>
...
//    sliderView.backgroundColor = [UIColor clearColor];
    ...
    _mesBtn.backgroundColor = [UIColor recordView:StringFromProcessorData(showValueIdent)];
...
//
//            lineView.backgroundColor = [UIColor colorWithPatternImage:[GradientHelper getLinearGradientImage:RGB_COLOR_String(kCommonBGColor_begin) and:RGB_COLOR_String(kCommonBGColor_end) directionType:SNLinearGradientDirectionLevel]];
//            button.selected = YES;
//        } else if (i == 1){
...
-(void)averaged:(UIButton *)sender{
    ...
    //: }else if (sender == self.resqBtn){
        ...
        _resqBtn.backgroundColor = [UIColor recordView:StringFromProcessorData(showValueIdent)];
        ...
        [_mesBtn setTitleColor:[UIColor recordView:StringFromProcessorData(notiShouldId)] forState:UIControlStateNormal];
    }


    //: if (self.delegate && [self.delegate respondsToSelector:@selector(sliderWithIndex:)]) {
    if (self.delegate && [self.delegate respondsToSelector:@selector(streetSmart:)]) {
        //: [self.delegate sliderWithIndex:sender.tag -200];
        [self.delegate streetSmart:sender.tag -200];
    }
...
-(void)circleClick:(UIButton *)sender{
    ...
    switch (tag) {
        ...
        case 0:
        {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(managerView)]) {
                //: [self.delegate friendNotification];
                [self.delegate managerView];
            }
        }
            //: break;
            break;
        ...
        case 2:
        {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(actionAnd)]) {
                //: [self.delegate blackFriendList];
                [self.delegate actionAnd];
            }
        }
            //: break;
            break;
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    ...
    if (self.delegate &&[self.delegate respondsToSelector:@selector(tipImage)]) {
        //: [self.delegate searchClick];
        [self.delegate tipImage];
    }
...
-(void)butylate:(NSInteger)systemCount{
...
Byte * ProcessorDataToCache(Byte *data) {
    int copernicanSystem = data[0];
    int tag = data[1];
    Byte vanguard = data[2];
    int content = data[3];
    if (!copernicanSystem) return data + content;
    for (int i = content; i < content + tag; i++) {
        int value = data[i] + vanguard;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
...
NSString *StringFromProcessorData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)ProcessorDataToCache(data)];
}
