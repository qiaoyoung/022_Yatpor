...
NSString *const user_labelPath = @"online_state";
...
@interface DistrictManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
+ (instancetype)nimThanManager
{
    ...
    static DistrictManager *instance;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESSubscribeManager alloc] init];
        instance = [[DistrictManager alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (void)toolState:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self cap];
...
- (void)phoneMessage
{
    ...
    NSDictionary *ext = @{
                            ...
                            noti_currentMinName : @([OnTingHandle modernIndicator].mottle),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            user_labelPath : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext jsonNameItem]];
...
- (void)subscribeKey
{
    ...
    [_subscribeIds addObjectsFromArray:self.scaleOfMeasurement.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_subscribeIds addObjectsFromArray:self.brush.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self option:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)option:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.scaleOfMeasurement containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.brush containsObject:recentSession.session.sessionId]) {
