...
static CGFloat const mPendingValue = 0.1;
//: static CGFloat const MIN_PRESS_TO_BEGIN_EDITING_DURATION = 0.6;
static CGFloat const dream_dataTitle = 0.6;

//: static inline CGPoint CGPointOffset(CGPoint point, CGFloat dx, CGFloat dy)
static inline CGPoint magnitudeerceptFile(CGPoint point, CGFloat dx, CGFloat dy)
{
...
@interface DateViewLayout () <UIGestureRecognizerDelegate>

//: @property (nonatomic,readonly) id<LxGridViewDataSource> dataSource;
@property (nonatomic,readonly) id<TingSend> dataSource;
//: @property (nonatomic,readonly) id<LxGridViewDelegateFlowLayout> delegate;
@property (nonatomic,readonly) id<EmptyLayout> delegate;

//: @end
@end

//: @implementation LxGridViewFlowLayout
@implementation DateViewLayout
{
...
- (void)empty
{
    ...
    _longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(stormCentre:)];
    //: _longPressGestureRecognizer.cancelsTouchesInView = NO;
    _longPressGestureRecognizer.cancelsTouchesInView = NO;
    //: _longPressGestureRecognizer.minimumPressDuration = PRESS_TO_MOVE_MIN_DURATION;
    _longPressGestureRecognizer.minimumPressDuration = mPendingValue;
    ...
    _panGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(textTriggerd:)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resigned:) name:UIApplicationWillResignActiveNotification object:nil];
}

//: - (void)removeGestureRecognizers
- (void)indexRecognizers
{
...
- (void)stormCentre:(UILongPressGestureRecognizer *)longPress
{
    ...
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (_displayLink == nil) {
                ...
                _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(tieIn:)];
                ...
                _remainSecondsToBeginEditing = dream_dataTitle;
            }
            ...
            if ([self.delegate respondsToSelector:@selector(color:asStart:center:)]) {
                //: [self.delegate collectionView:self.collectionView layout:self willBeginDraggingItemAtIndexPath:_movingItemIndexPath];
                [self.delegate color:self.collectionView asStart:self center:_movingItemIndexPath];
            }
            ...
            ReasonViewCell *sourceCell = (ReasonViewCell *)sourceCollectionViewCell;
            ...
            UIView * highlightedSnapshotView = [sourceCell mySearch];
            ...
            UIView * snapshotView = [sourceCell mySearch];
            ...
            [UIView animateWithDuration:0
                             ...
                             completion:^(BOOL finished) {
                                 ...
                                 if (strongSelf) {
                                     ...
                                     if ([strongSelf.delegate respondsToSelector:@selector(red:benefitPath:artCollectionInPath:)]) {
                                         ...
                                         [strongSelf.delegate red:strongSelf.collectionView benefitPath:strongSelf artCollectionInPath:self->_movingItemIndexPath];
                                     }
                                 }
                             //: }];
                             }];
        ...
        case UIGestureRecognizerStateCancelled:
        {
            ...
            if (movingItemIndexPath) {
                ...
                if ([self.delegate respondsToSelector:@selector(image:changePath:smorgasbordInsideWarpathWith:)]) {
                    //: [self.delegate collectionView:self.collectionView layout:self willEndDraggingItemAtIndexPath:movingItemIndexPath];
                    [self.delegate image:self.collectionView changePath:self smorgasbordInsideWarpathWith:movingItemIndexPath];
                }
                ...
                [UIView animateWithDuration:0.2
                                 ...
                                 completion:^(BOOL finished) {
                                     ...
                                     if (strongSelf) {
                                         ...
                                         if ([strongSelf.delegate respondsToSelector:@selector(title:alongOnPath:scaleOfMeasurement:)]) {
                                             ...
                                             [strongSelf.delegate title:strongSelf.collectionView alongOnPath:strongSelf scaleOfMeasurement:movingItemIndexPath];
                                         }
                                     }
                                 //: }];
                                 }];
...
- (void)textTriggerd:(UIPanGestureRecognizer *)pan
{
    //: switch (pan.state) {
    switch (pan.state) {
        //: case UIGestureRecognizerStatePossible:
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            _beingMovedPromptView.center = magnitudeerceptFile(_sourceItemCollectionViewCellCenter, panTranslation.x, panTranslation.y);
            ...
            if ([self.dataSource respondsToSelector:@selector(metadata:should:max:)] && [self.dataSource metadata:self.collectionView should:sourceIndexPath max:destinationIndexPath] == NO) {
                //: return;
                return;
            }

            //: if ([self.dataSource respondsToSelector:@selector(collectionView:itemAtIndexPath:willMoveToIndexPath:)]) {
            if ([self.dataSource respondsToSelector:@selector(cipherPath:size:tag:)]) {
                ...
                [self.dataSource cipherPath:self.collectionView size:sourceIndexPath tag:destinationIndexPath];
            }
            ...
            //: } completion:^(BOOL finished) {
                ...
                if ([strongSelf.dataSource respondsToSelector:@selector(ting:teamStop:indexFor:)]) {
                    ...
                    [strongSelf.dataSource ting:strongSelf.collectionView teamStop:sourceIndexPath indexFor:destinationIndexPath];
                }
            //: }];
            }];
        }
...
- (void)tieIn:(CADisplayLink *)displayLink
{
...
- (void)resigned:(NSNotification *)notificaiton
{
