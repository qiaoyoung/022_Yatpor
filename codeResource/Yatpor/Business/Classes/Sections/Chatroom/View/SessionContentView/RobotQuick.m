...
@interface EnableButton : UIButton
...
@implementation RobotQuick

//: - (instancetype)initSessionMessageContentView
- (instancetype)initNameView
{
...
- (void)bar:(NIMMessageModel *)data
{
    //: [super refresh:data];
    [super refresh:data];
    //: [self setupRobot:data];
    [self requestRobot:data];
}


//: - (UIImage *)chatBubbleImageForState:(UIControlState)state outgoing:(BOOL)outgoing
- (UIImage *)betweenDateOutgoing:(UIControlState)state scaleValue:(BOOL)outgoing
{
    //: return nil;
    return nil;
}
...
- (void)requestRobot:(NIMMessageModel *)data
{
...
- (void)containerTo:(NIMKitRobotTemplateItem *)item
           //: inView:(UIView *)view
           menu:(UIView *)view
{
    ...
    switch (item.itemType) {
        ...
        case NIMKitRobotTemplateItemTypeText:
        {
            ...
            WithSessionNameView *label = [self genLabel];
...
- (CGSize)send:(CGSize)size
{
    //: CGFloat height = [NTESSessionRobotContentView itemSpacing];
    CGFloat height = [RobotQuick itemSpacing];
    //: CGFloat width = 0;;
    CGFloat width = 0;;
    ...
    for (UIView *view in self.subviews) {
        ...
        if (view == self.continueButton)
        {
            ...
            height += [RobotQuick continueItemSpacing];
        }
        //: else
        else
        {
            //: height += [NTESSessionRobotContentView itemSpacing];
            height += [RobotQuick itemSpacing];
        }
    }
...
- (void)videoArea:(EnableButton *)button
{
    //: NIMKitEvent *event = [[NIMKitEvent alloc] init];
    NIMKitEvent *event = [[NIMKitEvent alloc] init];
    ...
    else
    {
        ...
        for (WithSessionNameView *label in button.subviews)
        {
            //: if ([label isKindOfClass:[M80AttributedLabel class]])
            if ([label isKindOfClass:[WithSessionNameView class]])
            {
...
- (void)stigmatiseEmpty
{
    ...
    for (UIView *view in self.subviews)
    {
        ...
        top = view.bottom + [RobotQuick itemSpacing];
    }
}


//: - (void)resizeAllSubView:(UIView *)superView insets:(UIEdgeInsets)insets
- (void)file:(UIView *)superView kibbutznik:(UIEdgeInsets)insets
{
    ...
    for (UIView *subView in superView.subviews)
    {
        ...
        if (subView.height == 0)
        {
            ...
            if ([subView isKindOfClass:[WithSessionNameView class]])
            {
                //: M80AttributedLabel *label = (M80AttributedLabel *)subView;
                WithSessionNameView *label = (WithSessionNameView *)subView;
            ...
            else if ([subView isKindOfClass:[EnableButton class]])
            {
                //: NTESSessionRobotButton *button = (NTESSessionRobotButton *)subView;
                EnableButton *button = (EnableButton *)subView;
                //: button.width = width;
                button.width = width;
...
- (void)press:(UIView *)view
{
    ...
    for (UIView *subView in view.subviews)
    {
        ...
        if ([subView isKindOfClass:[EnableButton class]])
        {
            //: NTESSessionRobotButton *btn = (NTESSessionRobotButton *)subView;
            EnableButton *btn = (EnableButton *)subView;
            //: btn.target = nil;
            btn.target = nil;
            //: btn.url = nil;
            btn.url = nil;
...
- (WithSessionNameView *)ting
{
    //: M80AttributedLabel *label = nil;
    WithSessionNameView *label = nil;
    ...
    else
    {
        ...
        label = [[WithSessionNameView alloc] initWithFrame:CGRectZero];
...
- (EnableButton *)gen
{
    ...
    else
    {
        ...
        button = [[EnableButton alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
