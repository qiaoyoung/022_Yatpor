...
@interface PerspectiveScrollView : UIScrollView
...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(blueDirection)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate blueDirection];
    }
...
- (void)lengthMargin
{
    ...
    if (_scrollView == nil)
    {
        ...
        _scrollView = [[PerspectiveScrollView alloc]initWithFrame:self.bounds];
...
- (NSInteger)max:(NSInteger)value image:(NSInteger)min device:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)visualProperty:(NSInteger)currentPage
{
    ...
    NSInteger first = [self max:currentPage - 1 image:0 device:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self max:currentPage + 1 image:0 device:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(activity:inputPage:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource activity:self inputPage:index];
...
- (void)quickShow
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(parameterred:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource parameterred:self];
    }
...
- (void)session
{
    ...
    [self visualProperty:_currentPage];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(yearred:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate yearred:self];
    }
}
...
- (void)manager
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(marginOfSafety:be:status:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate marginOfSafety:self
...
@implementation PerspectiveScrollView
//fix: swipe to delete in a UITableView which is embeded in a UIScrollView
//: - (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRequireFailureOfGestureRecognizer:(nonnull UIGestureRecognizer *)otherGestureRecognizer
- (BOOL)ask:(UIGestureRecognizer *)gestureRecognizer bubbleCross:(nonnull UIGestureRecognizer *)otherGestureRecognizer
{
