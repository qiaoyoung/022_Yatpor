...
@interface ParallelData : NSObject

@end

@implementation ParallelData

+ (Byte *)ParallelDataToCache:(Byte *)data {
    int manufacturer = data[0];
    Byte hi = data[1];
    int miniByAnimal = data[2];
    for (int i = miniByAnimal; i < miniByAnimal + manufacturer; i++) {
        int value = data[i] + hi;
        if (value > 255) {
            value -= 256;
        }
    ...
    data[miniByAnimal + manufacturer] = 0;
    return data + miniByAnimal;
}

+ (NSString *)StringFromParallelData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self ParallelDataToCache:data]];
}
...
+ (NSString *)appViewStr {
    ...
    if (!appViewStr) {
        Byte value[] = {6, 38, 9, 188, 68, 120, 131, 68, 56, 61, 73, 70, 73, 76, 77, 99};
        appViewStr = [self StringFromParallelData:value];
    }
    return appViewStr;
}

//: chromateAnimate
+ (NSString *)show_tacticText {
    /* static */ NSString *show_tacticText = nil;
    if (!show_tacticText) {
        Byte value[] = {15, 69, 5, 104, 43, 30, 35, 45, 42, 40, 28, 47, 32, 252, 41, 36, 40, 28, 47, 32, 126};
        show_tacticText = [self StringFromParallelData:value];
    }
    return show_tacticText;
}
...
+ (UIImage *)inheritSureSingular:(UIColor *)startColor front:(UIColor *)endColor should:(SNGradientDirection)directionType {
    ...
    return [self cell:startColor info:endColor timeGradientDirection:directionType app:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice bearDown]))];
}
...
+ (UIImage *)cellShow:(UIColor *)centerColor bubble:(UIColor *)outColor {
    ...
    return [self tap:centerColor praise:outColor needOption:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice bearDown]))];
}
...
+ (UIImage *)tap:(UIColor *)centerColor praise:(UIColor *)outColor needOption:(CGSize)size {
    ...
    [self read:gc overFull:path colorOf:centerColor.CGColor tab:outColor.CGColor];
    //: CGPathRelease(path);
    CGPathRelease(path);
...
+ (void)read:(CGContextRef)context
                      //: path:(CGPathRef)path
                      overFull:(CGPathRef)path
                //: startColor:(CGColorRef)startColor
                colorOf:(CGColorRef)startColor
                  //: endColor:(CGColorRef)endColor {
                  tab:(CGColorRef)endColor {
...
+ (void)custom:(UIView *)view {
    ...
    CAKeyframeAnimation *chromateAnimate = [self changeAnimation];

    //: [chromatoLayer addAnimation:chromateAnimate forKey:@"chromateAnimate"];
    [chromatoLayer addAnimation:chromateAnimate forKey:[ParallelData show_tacticText]];
...
+ (CAKeyframeAnimation *)changeAnimation {
    //: CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:@"colors"];
    CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:[ParallelData appViewStr]];
