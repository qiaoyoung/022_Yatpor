...
- (id)init
{
    ...
    if (self) {
        //: [self initialization];
        [self compartmentInitialization];
    }
 ...
 */
//: - (void)initialization
- (void)compartmentInitialization
{
    //: self.dotViewClass = [TAAnimatedDotView class];
    self.dotViewClass = [ToDotView class];
    //: self.spacingBetweenDots = kDefaultSpacingBetweenDots;
    self.spacingBetweenDots = noti_ofUrl;
    //: self.numberOfPages = kDefaultNumberOfPages;
    self.numberOfPages = notiTingStr;
    //: self.currentPage = kDefaultCurrentPage;
    self.currentPage = mDrawMessage;
    //: self.hidesForSinglePage = kDefaultHideForSinglePage;
    self.hidesForSinglePage = mLastKey;
    //: self.shouldResizeFromCenter = kDefaultShouldResizeFromCenter;
    self.shouldResizeFromCenter = user_inputErrorMessage;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(lightModel:phoneText:)]) {
            //: [self.delegate TAPageControl:self didSelectPageAtIndex:index];
            [self.delegate lightModel:self phoneText:index];
        }
    }
}
...
- (void)sizeToFit
{
    ...
    [self caseHistory:YES];
}


//: - (CGSize)sizeForNumberOfPages:(NSInteger)pageCount
- (CGSize)selected:(NSInteger)pageCount
{
...
- (void)anonymDots
{
    ...
    for (NSInteger i = 0; i < self.numberOfPages; i++) {
        ...
        [self cell:dot recent:i];
    }

    //: [self changeActivity:YES atIndex:self.currentPage];
    [self info:YES at:self.currentPage];
...
- (void)caseHistory:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self selected:self.numberOfPages];
...
- (void)cell:(UIView *)dot recent:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self selected:self.numberOfPages].width) / 2);
...
- (UIView *)sound
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[ToDotView class]] && self.dotColor) {
            //: ((TAAnimatedDotView *)dotView).dotColor = self.dotColor;
            ((ToDotView *)dotView).dotColor = self.dotColor;
        }
    //: } else {
    } else {
...
- (void)info:(BOOL)active at:(NSInteger)index
{
    //: if (self.dotViewClass) {
    if (self.dotViewClass) {
        //: TAAbstractDotView *abstractDotView = (TAAbstractDotView *)[self.dots objectAtIndex:index];
        WorldView *abstractDotView = (WorldView *)[self.dots objectAtIndex:index];
        //: if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
        if ([abstractDotView respondsToSelector:@selector(linkAdministrativeDistrict:)]) {
            //: [abstractDotView changeActivityState:active];
            [abstractDotView linkAdministrativeDistrict:active];
        //: } else {
        } else {
...
- (void)setCurrentPage:(NSInteger)currentPage
{
    ...
    [self info:NO at:_currentPage];
    //: _currentPage = currentPage;
    _currentPage = currentPage;
    // Post set
    //: [self changeActivity:YES atIndex:_currentPage];
    [self info:YES at:_currentPage];
}
