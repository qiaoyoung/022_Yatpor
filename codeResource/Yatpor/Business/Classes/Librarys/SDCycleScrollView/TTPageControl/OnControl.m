...
- (void)sizeToFit
{
    ...
    [self meanSolarDay:YES];
}

//: - (void)updateFrame:(BOOL)overrideExistingFrame
- (void)meanSolarDay:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self kindBubble:self.numberOfPages];
...
- (CGSize)kindBubble:(NSInteger)pageCount
{
...
- (void)camera
{
    ...
    for (NSInteger i = 0; i <= self.numberOfPages; i++) {
        ...
        [self risklessnessBoundIndex:dot refresh:i];
    }

    //: [self changeIndicatorViewLayout:self.currentPage];
    [self middleTextLayout:self.currentPage];
...
- (void)risklessnessBoundIndex:(UIView *)dot refresh:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self kindBubble:self.numberOfPages].width) / 2);
...
- (void)middleTextLayout:(CGFloat)page
{
...
- (void)setCurrentPage:(CGFloat)currentPage
{
    ...
    [self middleTextLayout:currentPage];
}
