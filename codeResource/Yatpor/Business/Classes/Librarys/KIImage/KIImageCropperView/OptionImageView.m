...
@interface ImageReplacementView : UIView {
//: @private
@private
    //: CGRect _maskRect;
    CGRect _maskRect;
...
- (void)setSelectSize:(CGSize)size;
//: - (CGSize)maskSize;
- (CGSize)selectSize;
//: @end
@end
...
@interface OptionImageView() {
    ...
    ImageReplacementView *_maskView;
...
- (ImageReplacementView *)maskView {
    //: if (_maskView == nil) {
    if (_maskView == nil) {
        //: _maskView = [[KIImageCropperMaskView alloc] initWithFrame:self.bounds];
        _maskView = [[ImageReplacementView alloc] initWithFrame:self.bounds];
...
- (UIImageView *)actionView {
    ...
    if (_imageView == nil) {
        ...
        [[self hold] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (void)latchkey:(KIImageCropRatio)ratio {
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image != image) {
        ...
        [[self actionView] setImage:_image];

        //: [[self imageView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
        [[self actionView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
...
- (void)flush {
    ...
    [[self hold] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max+5.0f];
    [[self hold] setMaximumZoomScale:max+5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self hold] setZoomScale:min animated:YES];

    //: [[self scrollView] setContentInset:_imageViewInsets];
    [[self hold] setContentInset:_imageViewInsets];
    //: [[self scrollView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
    [[self hold] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    [(ImageReplacementView*)[self maskView] setSelectSize:[self betweenSize]];
    //: [[self scrollView] setFrame:self.bounds];
    [[self hold] setFrame:self.bounds];
...
- (UIImage *)fallContent {
    ...
    CGFloat zoomScale = [self hold].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self hold].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self hold].contentOffset.y;
    ...
    image = [self send:image isColorPin:imgSize standard:YES];
...
- (UIImage*)send:(UIImage*)image isColorPin:(CGSize)newSize standard:(BOOL)opaque
{
