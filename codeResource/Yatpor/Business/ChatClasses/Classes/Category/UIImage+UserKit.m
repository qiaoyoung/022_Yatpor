...
typedef struct {
    Byte sufferOff;
    Byte *laundryRequest;
    unsigned int verticalAccount;
    Byte globule;
} DeepData;

NSString *StringFromDeepData(DeepData *data);


//: content
DeepData kScanFormat = (DeepData){114, (Byte []){17, 29, 28, 6, 23, 28, 6, 178}, 7, 209};

//: Chartlet
DeepData app_northernValueName = (DeepData){165, (Byte []){230, 205, 196, 215, 209, 201, 192, 209, 245}, 8, 169};

//: default
DeepData k_seniorMessage = (DeepData){81, (Byte []){53, 52, 55, 48, 36, 61, 37, 225}, 7, 189};
...
@implementation UIImage (UserKit)

//: + (UIImage *)nim_fetchChartlet:(NSString *)imageName chartletId:(NSString *)chartletId{
+ (UIImage *)message:(NSString *)imageName index:(NSString *)chartletId{
    //: if ([chartletId isEqualToString:@"default"]) {
    if ([chartletId isEqualToString:StringFromDeepData(&k_seniorMessage)]) {
        //: return [UIImage imageNamed:imageName];
        return [UIImage imageNamed:imageName];
    }
    //: NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",@"Chartlet",chartletId,@"content"];
    NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",StringFromDeepData(&app_northernValueName),chartletId,StringFromDeepData(&kScanFormat)];
    //先拿2倍图
...
- (UIImage *)change: (CGFloat)suggestPixels
{
...
- (UIImage *)tab: (CGFloat)maxPixels
{
    ...
    return [self scaleArray:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)nim_scaleToSize:(CGSize)newSize
- (UIImage *)scaleArray:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self leave:size];
}

//: - (UIImage *)nim_drawImageWithSize: (CGSize)size
- (UIImage *)leave: (CGSize)size
{
...
- (UIImage *)quick:(CGSize)targetSize
{
    // 裁剪两边
...
Byte *DeepDataToByte(DeepData *data) {
    if (data->globule < 122) return data->laundryRequest;
    for (int i = 0; i < data->verticalAccount; i++) {
        data->laundryRequest[i] ^= data->sufferOff;
    }
    data->laundryRequest[data->verticalAccount] = 0;
    data->globule = 32;
    return data->laundryRequest;
}

NSString *StringFromDeepData(DeepData *data) {
    return [NSString stringWithUTF8String:(char *)DeepDataToByte(data)];
}
