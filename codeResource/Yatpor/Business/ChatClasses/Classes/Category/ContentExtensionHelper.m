...
@implementation NSString (Ground)

//: static char NIMKitStringJsonDictionaryAddress;
static char appEventId;
...
- (NSDictionary *)alongText
{
    ...
    NSDictionary *dict = [objc_getAssociatedObject(self, &appEventId) copy];
    //: if (dict == nil) 
    if (dict == nil) //解析过一次后就缓存解析结果，避免多次解析
    {
        ...
        objc_setAssociatedObject(self,&appEventId,dict,OBJC_ASSOCIATION_COPY);
    }
    //: return dict;
    return dict;
...
@implementation NSDictionary (Ground)
