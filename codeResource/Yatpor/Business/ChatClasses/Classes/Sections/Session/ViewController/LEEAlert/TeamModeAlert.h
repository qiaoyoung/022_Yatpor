 ...
 *************************简要说明************************
 ...
 [TeamModeAlert configMainWindow:self.window];
 
 
 A 使用方法
 
 [TeamModeAlert alert].cofing.XXXXX.XXXXX.LeeShow();
 
 TipMobile 使用方法
 
 [TeamModeAlert actionSheet].cofing.XXXXX.XXXXX.LeeShow();
...
@interface TeamModeAlert: NSObject

/** 初始化 */

//: + (nonnull LEEAlertConfig *)alert;
+ (nonnull BarEnableBaseConfig *)shared;

//: + (nonnull LEEActionSheetConfig *)actionsheet;
+ (nonnull EqualBaseConfig *)can;

/** 获取Alert窗口 */
//: + (nonnull LEEAlertWindow *)getAlertWindow;
+ (nonnull AlertView *)notice;
...
+ (void)stepOn:(NSString *)identifier by:(BOOL)force isWalkTo:(void (^ _Nullable)(void))completionBlock;
...
+ (void)fullses:(void (^ _Nullable)(void))completionBlock;
...
@interface CropModel(Presentation)

/** 设置 层级 -> 格式: .LeePresentation([LengthTool windowLevel:1000] / [LengthTool viewController:xxxx]) */
...
@interface BottomBubble: NSObject
...
@interface DisableMessage: NSObject
...
@interface LengthTool: NSObject

//: + (LEEPresentation *)windowLevel: (UIWindowLevel)level;
+ (LengthTool *)back: (UIWindowLevel)level;

//: + (LEEPresentation *)viewController: (UIViewController *)controller;
+ (LengthTool *)resignVideo: (UIViewController *)controller;

//: @end
@end

//: @interface LEEBaseConfig: NSObject
@interface VisualImageSection: NSObject

//: @property (nonatomic, strong, nonnull) LEEBaseConfigModel *config;
@property (nonatomic, strong, nonnull) CropModel *config;

//: @end
@end

//: @interface LEEAlertConfig: LEEBaseConfig
@interface BarEnableBaseConfig: VisualImageSection

//: @end
@end

//: @interface LEEActionSheetConfig: LEEBaseConfig
@interface EqualBaseConfig: VisualImageSection

//: @end
@end

//: @interface LEEAlertWindow: UIWindow @end
@interface AlertView: UIWindow @end

//: @interface LEEBaseViewController: UIViewController @end
@interface UndersurfaceViewController: UIViewController @end

//: @interface LEEAlertViewController: LEEBaseViewController @end
@interface CipherViewController: UndersurfaceViewController @end

//: @interface LEEActionSheetViewController: LEEBaseViewController @end
@interface DisableDoingViewController: UndersurfaceViewController @end


//: @interface UIView (CornerRadii)
@interface UIView (AIcon)
...
AIcon indexMode(void);

//: CornerRadii CornerRadiiNull(void);
AIcon cornerViewNull(void);
