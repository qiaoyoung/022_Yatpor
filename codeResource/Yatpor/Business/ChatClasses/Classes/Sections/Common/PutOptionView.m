...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self tweakUponContextShould:rect subTitle:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self findUser:rect value:context];
    }
...
- (CGSize)badgeWithBlue:(NSString *)badgeValue{
...
- (CGRect)frame:(NSString *)badgeValue{
    ...
    CGSize badgeSize = [self badgeWithBlue:badgeValue];
...
- (void)tweakUponContextShould:(CGRect)rect subTitle:(CGContextRef)context{
...
- (void)findUser:(CGRect)rect value:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
