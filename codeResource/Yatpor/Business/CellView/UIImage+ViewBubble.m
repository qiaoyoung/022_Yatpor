...
@implementation UIImage (ViewBubble)
...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(barCellMedia:));
...
+ (UIImage *)barCellMedia:(NSString *)name {
    ...
    UIImage *image = [[EqualArchiveManager message] barCellMedia:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self barCellMedia:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
