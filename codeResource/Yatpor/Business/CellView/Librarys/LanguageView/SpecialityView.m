...
#import "SpecialityView.h"

//: static CGFloat const DEFAULT_MASK_ALPHA = 0.75;
static CGFloat const dream_withImageKey = 0.75;
//: static BOOL const square = NO;
static BOOL const show_commonUrl = NO;
//: float IMAGE_MIN_HEIGHT = 400;
float m_colorFormat = 400;
//: float IMAGE_MIN_WIDTH = 400;
float main_indexUrl = 400;

//: @interface ImageCropViewController(){
@interface ToolTitleViewController(){
    //: CGRect _cropArea;
    CGRect _cropArea;
}
...
#pragma mark ToolTitleViewController implementation
//: @implementation ImageCropViewController
@implementation ToolTitleViewController
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        self.cropView = [[SpecialityView alloc] initWith:view blur:self.blurredBackground];
...
- (void)quickView:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(origin:of:)])
    {
        ...
        [self.delegate origin:self of:cropped];
    }
...
#pragma mark SendView implementation
...
@implementation StatuteContentView
...
static CGFloat const main_playPath = 5;
...
- (void)initName {
    ...
    self.shadeView = [[StatuteContentView alloc] initWithFrame:subviewFrame];
    ...
    controlPointSize = main_playPath;
    ...
    topLeftPoint = [self notice:singleCell(centerInView.x - initialCropAreaSize,
    ...
    bottomLeftPoint = [self notice:singleCell(centerInView.x - initialCropAreaSize,
                                                                      //: centerInView.y + initialCropAreaSize,
                                                                      centerInView.y + initialCropAreaSize,
                                                                      //: controlPointSize)];
                                                                      controlPointSize)];
    ...
    bottomRightPoint = [self notice:singleCell(centerInView.x + initialCropAreaSize,
    ...
    topRightPoint = [self notice:singleCell(centerInView.x + initialCropAreaSize,
                                                                    //: centerInView.y - initialCropAreaSize, controlPointSize)];
                                                                    centerInView.y - initialCropAreaSize, controlPointSize)];

    //the "hole"
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self path];
    ...
    UIPanGestureRecognizer* dragRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(locationned:)];
    ...
    self.maskAlpha = dream_withImageKey;
...
- (SendView*)notice:(CGRect)frame {
    //: ControlPointView* point = [[ControlPointView alloc] initWithFrame:frame];
    SendView* point = [[SendView alloc] initWithFrame:frame];
    //: return point;
    return point;
}
...
- (void)removeRight:(CGPoint)topLeft voice:(CGPoint)bottomLeft brink:(CGPoint)bottomRight than:(CGPoint)topRight {
...
- (void)locationned:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan || multiDragPoint.lastCount != count) {
        ...
        if (count > 1)
            ...
            [self manager:recognizer message:count];
        //: else
        else
            //: [self prepSingleTouchPan:recognizer];
            [self video:recognizer];
    ...
    if (count > 1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            [self show:[recognizer locationOfTouch:i inView:self] markDownBubble:(i == 0 ? dragViewOne : dragViewTwo)];
    ...
    //: } else {
        ...
        [self show:[recognizer locationInView:self] markDownBubble:dragViewOne];
    }
...
- (void)manager:(UIPanGestureRecognizer*)recognizer message:(NSUInteger)count
{
    //: for (int i = 0; i < count; i++) {
    for (int i = 0; i < count; i++) {
        //: if (i == 0) {
        if (i == 0) {
            ...
            dragViewOne = [self fileComment:[recognizer locationOfTouch:i inView:self]];
        ...
        //: } else {
            ...
            dragViewTwo = [self fileComment:[recognizer locationOfTouch:i inView:self]];
...
- (void)video:(UIPanGestureRecognizer*)recognizer
{
    //: dragViewOne = [self checkHit:[recognizer locationInView:self]];
    dragViewOne = [self fileComment:[recognizer locationInView:self]];
...
- (void)setFullLength:(CGRect)cropArea
{
...
- (void)show:(CGPoint)location markDownBubble:(UIView*)view
{
...
- (CGSize)equalCreate:(CGPoint)dragLocation be:(CGPoint)draggedPoint textPoint:(CGPoint)oppositePoint {
...
- (void)background:(CGPoint)dragLocation {
    ...
    CGSize disp = [self equalCreate:dragLocation be:dragPoint.topLeftCenter textPoint:dragPoint.bottomRightCenter];
    ...
    if (width >= main_indexUrl && height < m_colorFormat) {
        //: topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - IMAGE_MIN_HEIGHT) / self.imageScale);
        topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - m_colorFormat) / self.imageScale);
        //: topRight.y = topLeft.y;
        topRight.y = topLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < main_indexUrl && height >= m_colorFormat) {
        //: topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - main_indexUrl) / self.imageScale);
    ...
    [self removeRight:topLeft voice:bottomLeft brink:dragPoint.bottomRightCenter than:topRight];
}
//: - (void)handleDragBottomLeft:(CGPoint)dragLocation {
- (void)nameTing:(CGPoint)dragLocation {
    ...
    CGSize disp = [self equalCreate:dragLocation be:dragPoint.bottomLeftCenter textPoint:dragPoint.topRightCenter];
    ...
    if (width >= main_indexUrl && height < m_colorFormat) {
        ...
        bottomLeft.y = cropArea.origin.y + (m_colorFormat / self.imageScale);
        //: bottomRight.y = bottomLeft.y;
        bottomRight.y = bottomLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < main_indexUrl && height >= m_colorFormat) {
        //: bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - main_indexUrl) / self.imageScale);
    ...
    [self removeRight:topLeft voice:bottomLeft brink:bottomRight than:dragPoint.topRightCenter];
}

//: - (void)handleDragBottomRight:(CGPoint)dragLocation {
- (void)handleMember:(CGPoint)dragLocation {
    ...
    CGSize disp = [self equalCreate:dragLocation be:dragPoint.bottomRightCenter textPoint:dragPoint.topLeftCenter];
    ...
    if (width >= main_indexUrl && height < m_colorFormat) {
        ...
        bottomRight.y = cropArea.origin.y + (m_colorFormat / self.imageScale);
        //: bottomLeft.y = bottomRight.y;
        bottomLeft.y = bottomRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < main_indexUrl && height >= m_colorFormat) {
        //: bottomRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        bottomRight.x = cropArea.origin.x + (main_indexUrl / self.imageScale);
    ...
    [self removeRight:dragPoint.topLeftCenter voice:bottomLeft brink:bottomRight than:topRight];
}

//: - (void)handleDragTopRight:(CGPoint)dragLocation {
- (void)isShow:(CGPoint)dragLocation {
    ...
    CGSize disp = [self equalCreate:dragLocation be:dragPoint.topRightCenter textPoint:dragPoint.bottomLeftCenter];
    ...
    if (width >= main_indexUrl && height < m_colorFormat) {
        ...
        topRight.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - m_colorFormat) / self.imageScale);
        //: topLeft.y = topRight.y;
        topLeft.y = topRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < main_indexUrl && height >= m_colorFormat) {
        //: topRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        topRight.x = cropArea.origin.x + (main_indexUrl / self.imageScale);
    ...
    [self removeRight:topLeft voice:dragPoint.bottomLeftCenter brink:bottomRight than:topRight];
}

//: - (void)handleDragCropArea:(CGPoint)dragLocation {
- (void)byCover:(CGPoint)dragLocation {
    //: CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    //: CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
    CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
...
- (void)setImage:(UIImage *)image {
    ...
    if (self.blurred) {
        ...
        blur = [image thoughtImage:30 iterations:1 countervalRange:[UIColor blackColor]];
    //: } else {
    } else {
        //: blur = [image blurredImageWithRadius:0 iterations:1 tintColor:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
        blur = [image thoughtImage:0 iterations:1 countervalRange:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
    }
    ...
    [self removeRight:topLeftPoint.center voice:bottomLeftPoint.center brink:bottomRightPoint.center than:topRightPoint.center];
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self path];
    //: cropAreaView.frame = cropArea;
    cropAreaView.frame = cropArea;
...
@implementation UIImage (OrientationSession)
