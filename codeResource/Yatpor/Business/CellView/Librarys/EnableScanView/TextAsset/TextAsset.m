...
@interface TextAsset () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
    //: SGSoundEffect *soundEffect;
    ColorEffect *soundEffect;
}
...
- (void)setDelegate:(id<SuccessGlassDelegate>)delegate {
    //: _delegate = delegate;
    _delegate = delegate;
...
- (void)setSampleBufferDelegate:(id<NameStyle>)sampleBufferDelegate {
...
- (void)user:(UIImage *)image tipWith:(void (^)(NSString *result))completion {
...
- (void)brisancePost:(NSString *)name {
    ...
    NSString *voicePath = [[[EqualArchiveManager message] container] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    soundEffect = [ColorEffect soundTime:voicePath];
    //: [soundEffect play];
...
- (AVCaptureSession *)session {
    ...
    if (!_session) {
        ...
        _session.sessionPreset = [self shadow];
    }
    //: return _session;
    return _session;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(holder:key:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate holder:self key:resultString];
            }
        //: });
        });
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(device:along:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate device:self along:brightnessValue];
        }
    //: });
    });
}
