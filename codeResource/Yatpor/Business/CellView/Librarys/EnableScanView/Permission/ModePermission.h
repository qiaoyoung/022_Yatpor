...
typedef void(^SGPermissionBlock)(ModePermission *permission, SGPermissionStatus status);

//: @interface SGPermission : NSObject
@interface ModePermission : NSObject
/// 对象方法获取权限状态
///
/// @param type        权限类型
/// @param block       权限状态回调
...
- (void)redKey:(void (^)(BOOL granted))handler;
