...
@implementation ReasonSession

//: + (NSString *)fileMD5:(NSString *)filepath
+ (NSString *)languageChild:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self byBubble:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self convertMd5:filepath];
        //: }else {
        }else {
...
+ (void)at:(NSString *)filepath edgeView:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self languageChild:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)convertMd5:(NSString *)filePath
{
...
+ (unsigned long long)byBubble:(NSString *)filepath
{
