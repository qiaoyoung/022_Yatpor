...
- (Byte *)UserDataToCache:(Byte *)data {
    int comfort = data[0];
    Byte blueWitnessFrame = data[1];
    int argument = data[2];
    for (int i = argument; i < argument + comfort; i++) {
        int value = data[i] - blueWitnessFrame;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[argument + comfort] = 0;
...
//: yyyy-MM-dd HH:mm:ss
- (NSString *)showBarUrl {
    /* static */ NSString *showBarUrl = nil;
    if (!showBarUrl) {
		NSString *origin = @"135F0A916CB1406E2816D8D8D8D88CACAC8CC3C37FA7A799CCCC99D2D246";
		NSData *data = [UserData UserDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        showBarUrl = [self StringFromUserData:value];
    }
    return showBarUrl;
}

//: png
- (NSString *)dream_thickStereoGrayIdent {
    /* static */ NSString *dream_thickStereoGrayIdent = nil;
    if (!dream_thickStereoGrayIdent) {
		NSString *origin = @"03610B954B27669E681C8AD1CFC8AD";
		NSData *data = [UserData UserDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        dream_thickStereoGrayIdent = [self StringFromUserData:value];
    }
    return dream_thickStereoGrayIdent;
}

//: gif
- (NSString *)noti_backId {
    /* static */ NSString *noti_backId = nil;
    if (!noti_backId) {
		NSString *origin = @"033407164DFC859B9D9AC1";
		NSData *data = [UserData UserDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        noti_backId = [self StringFromUserData:value];
    }
    return noti_backId;
}
...
- (NSString *)dreamTableData {
    ...
    if (!dreamTableData) {
		NSString *origin = @"044B04A9BFB4B1B165";
		NSData *data = [UserData UserDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        dreamTableData = [self StringFromUserData:value];
    }
    return dreamTableData;
}

//: jpeg
- (NSString *)notiNeedFormat {
    /* static */ NSString *notiNeedFormat = nil;
    if (!notiNeedFormat) {
		NSString *origin = @"04320A484D094D8E9E6C9CA2979945";
		NSData *data = [UserData UserDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        notiNeedFormat = [self StringFromUserData:value];
    }
    return notiNeedFormat;
}
...
- (UIImage *)digitizer:(BOOL)isHorizontal {
...
- (UIImage *)holderPin:(CGFloat)width {
    ...
    return [self wordName:width dismiss:height];
}
...
- (UIImage *)viewStatus:(CGFloat)heigh {
    ...
    return [self wordName:width dismiss:heigh];
}
...
- (UIImage *)designate {
    ...
    [df setDateFormat:[[UserData sharedInstance] showBarUrl]];
    ...
    return [self background:dateString
               //: textColor:[UIColor blackColor]
               to:[UIColor blackColor]
                    //: font:[UIFont boldSystemFontOfSize:16.0f]
                    name_strong:[UIFont boldSystemFontOfSize:16.0f]
                   //: point:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
                   with:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
...
+ (NSString *) itemData:(NSData *)data{
    ...
    switch (c) {
        ...
        case 0xFF:
            //: return @"jpeg";
            return [[UserData sharedInstance] notiNeedFormat];
        //: case 0x89:
        case 0x89:
            //: return @"png";
            return [[UserData sharedInstance] dream_thickStereoGrayIdent];
        //: case 0x47:
        case 0x47:
            //: return @"gif";
            return [[UserData sharedInstance] noti_backId];
        //: case 0x49:
        case 0x49:
        //: case 0x4D:
        case 0x4D:
            //: return @"tiff";
            return [[UserData sharedInstance] dreamTableData];
    }
    //: return @"";
    return @"";
}
...
CGFloat sizePush(CGFloat degrees) {return degrees * 3.14159265358979323846264338327950288 / 180;};
//: CGFloat KKRadiansToDegrees(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
CGFloat indexTip(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
...
- (UIImage *)event:(CGFloat)degrees{
    ...
    CGAffineTransform t = CGAffineTransformMakeRotation(sizePush(degrees));
    ...
    CGContextRotateCTM(bitmap, sizePush(degrees));
...
- (UIImage*)session:(CGSize)targetSize
{
    //: UIImage *sourceImage = self;
    UIImage *sourceImage = self;
    //: UIImage *newImage = nil;
...
- (UIImage *)parent:(NSUInteger)maxLength{
    ...
    NSData *compressData = [self house:maxLength*1024];
