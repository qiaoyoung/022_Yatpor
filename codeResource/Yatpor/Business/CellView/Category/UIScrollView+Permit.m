...
typedef struct {
    Byte respective;
    Byte *fixing;
    unsigned int ptolemaicSystemTag;
	int bronchitic;
...
//: Pull to refresh...
@property (nonatomic, copy) NSString *user_lestKey;

//: 下拉加载...
@property (nonatomic, copy) NSString *m_textMessage;

//: frame
@property (nonatomic, copy) NSString *dreamCollectionMsg;

//: SVPullToRefreshView
@property (nonatomic, copy) NSString *main_remoteIdent;

//: 加载中...
@property (nonatomic, copy) NSString *userScaleImageStr;

//: contentSize
@property (nonatomic, copy) NSString *app_tagImageUrl;

//: 松开刷新...
@property (nonatomic, copy) NSString *userAnxiousName;

//: contentOffset
@property (nonatomic, copy) NSString *main_commentKey;
...
+ (instancetype)sharedInstance {
    static AccumulationData *instance = nil;
...
- (Byte *)AccumulationDataToByte:(StructAccumulationData *)data {
    for (int i = 0; i < data->ptolemaicSystemTag; i++) {
        data->fixing[i] ^= data->respective;
    }
    data->fixing[data->ptolemaicSystemTag] = 0;
	if (data->ptolemaicSystemTag >= 1) {
		data->bronchitic = data->fixing[0];
	}
    return data->fixing;
}
...
//: 下拉加载...
- (NSString *)m_textMessage {
    if (!_m_textMessage) {
        StructAccumulationData value = (StructAccumulationData){85, (Byte []){177, 237, 222, 179, 222, 220, 176, 223, 245, 189, 232, 232, 123, 123, 123, 48}, 15, 232};
        _m_textMessage = [self StringFromAccumulationData:&value];
    }
    return _m_textMessage;
}

//: 加载中...
- (NSString *)userScaleImageStr {
    if (!_userScaleImageStr) {
        StructAccumulationData value = (StructAccumulationData){16, (Byte []){245, 154, 176, 248, 173, 173, 244, 168, 189, 62, 62, 62, 209}, 12, 91};
        _userScaleImageStr = [self StringFromAccumulationData:&value];
    }
    return _userScaleImageStr;
}

//: SVPullToRefreshView
- (NSString *)main_remoteIdent {
    if (!_main_remoteIdent) {
        StructAccumulationData value = (StructAccumulationData){50, (Byte []){97, 100, 98, 71, 94, 94, 102, 93, 96, 87, 84, 64, 87, 65, 90, 100, 91, 87, 69, 221}, 19, 122};
        _main_remoteIdent = [self StringFromAccumulationData:&value];
    }
    return _main_remoteIdent;
}

//: contentSize
- (NSString *)app_tagImageUrl {
    if (!_app_tagImageUrl) {
        StructAccumulationData value = (StructAccumulationData){202, (Byte []){169, 165, 164, 190, 175, 164, 190, 153, 163, 176, 175, 235}, 11, 151};
        _app_tagImageUrl = [self StringFromAccumulationData:&value];
    }
    return _app_tagImageUrl;
}

//: 松开刷新...
- (NSString *)userAnxiousName {
    if (!_userAnxiousName) {
        StructAccumulationData value = (StructAccumulationData){224, (Byte []){6, 125, 94, 5, 92, 96, 5, 104, 87, 6, 118, 80, 206, 206, 206, 121}, 15, 170};
        _userAnxiousName = [self StringFromAccumulationData:&value];
    }
    return _userAnxiousName;
}

//: Pull to refresh...
- (NSString *)user_lestKey {
    if (!_user_lestKey) {
        StructAccumulationData value = (StructAccumulationData){232, (Byte []){184, 157, 132, 132, 200, 156, 135, 200, 154, 141, 142, 154, 141, 155, 128, 198, 198, 198, 116}, 18, 102};
        _user_lestKey = [self StringFromAccumulationData:&value];
    }
    return _user_lestKey;
}

//: frame
- (NSString *)dreamCollectionMsg {
    if (!_dreamCollectionMsg) {
        StructAccumulationData value = (StructAccumulationData){148, (Byte []){242, 230, 245, 249, 241, 57}, 5, 96};
        _dreamCollectionMsg = [self StringFromAccumulationData:&value];
    }
    return _dreamCollectionMsg;
}

//: contentOffset
- (NSString *)main_commentKey {
    if (!_main_commentKey) {
        StructAccumulationData value = (StructAccumulationData){65, (Byte []){34, 46, 47, 53, 36, 47, 53, 14, 39, 39, 50, 36, 53, 51}, 13, 152};
        _main_commentKey = [self StringFromAccumulationData:&value];
    }
    return _main_commentKey;
}
...
#import "UIScrollView+Permit.h"
//: #import <objc/runtime.h>
#import <objc/runtime.h>

//: static CGFloat const NTESPullToRefreshViewHeight = 60;
static CGFloat const userViaDateKey = 60;
...
@property (nonatomic, strong) PointerView *arrow;
...
- (void)setSumroduceForHigh:(UIEdgeInsets)insets;
//: - (void)rotateArrow:(float)degrees hide:(BOOL)hide;
- (void)shared:(float)degrees teamOn:(BOOL)hide;
...
#pragma mark - UIScrollView (Permit)


//: static char UIScrollViewPullToRefreshView;
static char userBlockText;

//: @implementation UIScrollView (NTESPullToRefresh)
@implementation UIScrollView (Permit)
...
- (void)quickPosition:(void (^)(void))actionHandler pull:(NTESPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        BirdSEyeView *view = [[BirdSEyeView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, userViaDateKey)];
...
- (void)isTo:(void (^)(void))actionHandler {
    ...
    [self quickPosition:actionHandler pull:NTESPullToRefreshPositionTop];
}
...
- (void)client {
    ...
    [self.pullToRefreshView record];
}

//: - (void)setPullToRefreshView:(NTESPullToRefreshView *)pullToRefreshView {
- (void)setPullToRefreshView:(BirdSEyeView *)pullToRefreshView {
    //: [self willChangeValueForKey:@"SVPullToRefreshView"];
    [self willChangeValueForKey:[AccumulationData sharedInstance].main_remoteIdent];
    //: objc_setAssociatedObject(self, &UIScrollViewPullToRefreshView,
    objc_setAssociatedObject(self, &userBlockText,
                             //: pullToRefreshView,
                             pullToRefreshView,
                             //: OBJC_ASSOCIATION_ASSIGN);
                             OBJC_ASSOCIATION_ASSIGN);
    //: [self didChangeValueForKey:@"SVPullToRefreshView"];
    [self didChangeValueForKey:[AccumulationData sharedInstance].main_remoteIdent];
}

//: - (NTESPullToRefreshView *)pullToRefreshView {
- (BirdSEyeView *)pullToRefreshView {
    //: return objc_getAssociatedObject(self, &UIScrollViewPullToRefreshView);
    return objc_getAssociatedObject(self, &userBlockText);
}
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (self.pullToRefreshView.isObserving) {
            ...
            [self removeObserver:self.pullToRefreshView forKeyPath:[AccumulationData sharedInstance].main_commentKey];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"contentSize"];
            [self removeObserver:self.pullToRefreshView forKeyPath:[AccumulationData sharedInstance].app_tagImageUrl];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"frame"];
            [self removeObserver:self.pullToRefreshView forKeyPath:[AccumulationData sharedInstance].dreamCollectionMsg];
            //: [self.pullToRefreshView resetScrollViewContentInset];
            [self.pullToRefreshView inset];
    ...
    //: else {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            [self addObserver:self.pullToRefreshView forKeyPath:[AccumulationData sharedInstance].main_commentKey options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"contentSize" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:[AccumulationData sharedInstance].app_tagImageUrl options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"frame" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:[AccumulationData sharedInstance].dreamCollectionMsg options:NSKeyValueObservingOptionNew context:nil];
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, userViaDateKey);
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString([AccumulationData sharedInstance].m_textMessage,),
                       //: NSLocalizedString(@"松开刷新...",),
                       NSLocalizedString([AccumulationData sharedInstance].userAnxiousName,),
                       //: NSLocalizedString(@"加载中...",),
                       NSLocalizedString([AccumulationData sharedInstance].userScaleImageStr,),
                       //: nil];
                       nil];
    ...
    if (self.superview && newSuperview == nil) {
        ...
        if (scrollView.showsPullToRefresh) {
            ...
            if (self.isObserving) {
                ...
                [scrollView removeObserver:self forKeyPath:[AccumulationData sharedInstance].main_commentKey];
                //: [scrollView removeObserver:self forKeyPath:@"contentSize"];
                [scrollView removeObserver:self forKeyPath:[AccumulationData sharedInstance].app_tagImageUrl];
                //: [scrollView removeObserver:self forKeyPath:@"frame"];
                [scrollView removeObserver:self forKeyPath:[AccumulationData sharedInstance].dreamCollectionMsg];
                //: self.isObserving = NO;
                self.isObserving = NO;
            }
        }
            ...
            case NTESPullToRefreshStateStopped:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self shared:0 teamOn:NO];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:NO];
                        [self shared:(float)3.14159265358979323846264338327950288 teamOn:NO];
                        //: break;
                        break;
                }
                //: break;
            ...
            case NTESPullToRefreshStateLoading:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self shared:0 teamOn:YES];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:YES];
                        [self shared:(float)3.14159265358979323846264338327950288 teamOn:YES];
                        //: break;
                        break;
                }
                //: break;
...
- (void)inset {
    ...
    [self setSumroduceForHigh:currentInsets];
}
...
- (void)setSumroduceForHigh:(UIEdgeInsets)contentInset {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if([keyPath isEqualToString:[AccumulationData sharedInstance].main_commentKey])
        //: [self scrollViewDidScroll:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
        [self infoManager:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
    //: else if([keyPath isEqualToString:@"contentSize"]) {
    else if([keyPath isEqualToString:[AccumulationData sharedInstance].app_tagImageUrl]) {
        //: [self layoutSubviews];
        [self layoutSubviews];
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, userViaDateKey);
    }
    //: else if([keyPath isEqualToString:@"frame"])
    else if([keyPath isEqualToString:[AccumulationData sharedInstance].dreamCollectionMsg])
        //: [self layoutSubviews];
        [self layoutSubviews];
...
- (void)infoManager:(CGPoint)contentOffset {
...
- (PointerView *)arrow {
    ...
    if(!_arrow) {
        ...
        _arrow = [[PointerView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (UILabel *)titleLabel {
    ...
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString([AccumulationData sharedInstance].user_lestKey,);
