...
@property (nonatomic,weak) id<DisableStyleDelegate> delegate;
                         ...
                         showSkullSessionText:(id<SaloonConfig>)sessionConfig
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(providering:) name:UIMenuControllerDidHideMenuNotification object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(theOption:) name:appFieldMsg object:nil];
    }
    //: return self;
    return self;
}
...
- (void)textView
{
    ...
    self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice bearDown]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice bearDown])-self.inputView.height);
    //: self.tableView.bottom = self.inputView.nim_top;
    self.tableView.bottom = self.inputView.nim_top;
    //: [self.tableView nim_scrollToBottom:YES];
    [self.tableView digitiserLabel:YES];
...
- (void)block
{
    ...
    [self.refreshControl addTarget:self action:@selector(oned:) forControlEvents:UIControlEventValueChanged];
...
- (void)oned:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(enableRefresh)])
    {
        //: [self.delegate onRefresh];
        [self.delegate enableRefresh];
    }
}

//: - (void)insert:(NSArray<NSIndexPath *> *)indexPaths animated:(BOOL)animated
- (void)valueKey:(NSArray<NSIndexPath *> *)indexPaths begin:(BOOL)animated
{
    //: if (!indexPaths.count)
    if (!indexPaths.count)
    {
...
//        }];
    ...
    if (isScroll || [self under:addIndexPathes])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
    }
...
- (void)versions:(NSArray<NSIndexPath *> *)indexPaths
{
    //: if ([self shouldReloadWhenRemoveOrUpdate:indexPaths])
    if ([self highShould:indexPaths])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
        //: return;
...
- (void)dismissBy {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!self.inputView.replyedContent.hidden)
        {
            ...
            [self.inputView.replyedContent of];
        }
    //: });
    });
}
...
- (BOOL)under:(NSArray<NSIndexPath *> *)indexPaths
{
...
- (BOOL)highShould:(NSArray<NSIndexPath *> *)indexPaths
{
    //: for(NSIndexPath * indexPath in indexPaths)
    for(NSIndexPath * indexPath in indexPaths)
    {
